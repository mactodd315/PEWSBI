...
if __name__ == "__main__":
    ...
    posteriors = numpy.ndarray((n_parameters, len(samples), args.n_bins))
    with h5py.File(args.output_folder+'/'+posteriorname, 'w') as f:
        for i in range(n_parameters):
            posteriors = numpy.ndarray((len(samples), args.n_bins))
            for j in range(len(samples)):
                posteriors[j,:] = numpy.histogram(samples[j], args.n_bins,
                range=[bounds[0][i],bounds[1][i]],
                density=True)[0]*(bounds[1][i]-bounds[0][i])/args.n_bins
            f['posteriors/'+parameters[i]] = posteriors
            f['bounds/'+parameters[i]] = [bounds[0][i], bounds[1][i]]
            f['true_parameters/'+parameters[i]] = true_parameters[i]
            
    if args.verbose:  print("All posterior estimates written.")
